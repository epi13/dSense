#!/usr/bin/env python
"""
Generate system activity scenarios for dSense baseline expansion.

Records scenarios with controlled system activity (CPU, I/O, network, thermal)
to create a comprehensive "system under load" baseline.

Usage:
    python scripts/generate_system_activity_dataset.py <project_name> [OPTIONS]
...
from scenarios import ACTIVITY_SCENARIOS


def print_scenarios(filter_str: str = None) -> None:
    """Print all available activity scenarios."""
    print(f"\n{'='*80}")
    print("Available System Activity Scenarios")
    print(f"{'='*80}\n")
    
    filtered = ACTIVITY_SCENARIOS
    if filter_str:
        filtered = [s for s in ACTIVITY_SCENARIOS if filter_str.lower() in s.label.lower()]
...
def record_activity_dataset(project_name: str, tick_hz: int = 100, filter_str: str = None, 
                           dry_run: bool = False, verbose: bool = False) -> dict:
    """
    Record system activity scenarios for a project.
    ...
    scenarios = ACTIVITY_SCENARIOS
    if filter_str:
        scenarios = [s for s in ACTIVITY_SCENARIOS if filter_str.lower() in s.label.lower()]
...
def main():
    parser = argparse.ArgumentParser(
        prog="generate_system_activity_dataset.py",
        description="Auto-generate system activity baseline scenarios for dSense",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Generate all activity scenarios
  python scripts/generate_system_activity_dataset.py demo_lab
  
  # Generate only CPU-related scenarios
  python scripts/generate_system_activity_dataset.py demo_lab --filter cpu
  ...
  python scripts/generate_system_activity_dataset.py --list
    ...
    # Record the activity dataset
    try:
        record_activity_dataset(
            args.project_name,
