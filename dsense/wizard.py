...
def guided_scene(project: str, label: str, duration: float, pre_roll: float, action: float, post_roll: float,
                 repeat: int = 1, notes: str = "", tick_hz: int = 100, assume_yes: bool = False) -> list[dict[str, object]]:
    results = []
    for r in range(1, repeat + 1):
        ...
        scene = record_scene(scene_dir, scene_id, label, duration, tick_hz, pre_roll, action, post_roll, notes)
        print("Done.")
